...
import { warn, formatComponentName } from './debug'
...
function logError(err, vm, info) {
  if (__DEV__) {
    const componentName = vm ? formatComponentName(vm, false) : ''
    const msg = `[Vue Error]${componentName ? ` in ${componentName.replace(/[<>]/g, '')}` : ''}: ${err.toString()} (found in ${info})`
