 ...
 */
const dynamicComponents: Record<string, any> = Object.create(null)

/**
 * Reactive version counter to trigger re-renders
 */
let dynamicComponentsVersion = 0

/**
 * Keep track of all Vue instances for automatic re-rendering
 */
const vueInstances: Array<any> = []
 ...
 */
export function defineDynamicComponent(name: string, component: any, options: { forceUpdate?: boolean } = {}): void {
  dynamicComponents[name] = component
  // Also register camelized and PascalCase variations
  const camelized = camelize(name)
  const pascal = capitalize(camelized)
  dynamicComponents[camelized] = component
  dynamicComponents[pascal] = component
  
  // Increment version to trigger reactivity
  dynamicComponentsVersion++
  
  // Determine if we should force update
  const shouldForceUpdate = options.forceUpdate !== undefined ? options.forceUpdate : autoForceUpdate
  
  debugLog(`[Vue.defineDynamicComponent] Registered "${name}", found ${vueInstances.length} instances`)
  
  // Only re-render instances that actually need this component
  // This is more efficient than broadcasting to all instances
  if (shouldForceUpdate) {
    vueInstances.forEach((vm, index) => {
      debugLog(`[Vue.defineDynamicComponent] Instance ${index}: $forceUpdate=${!!vm.$forceUpdate}, _watcher=${!!vm._watcher}`)
...
/**
 * Enable or disable automatic forceUpdate when registering dynamic components
 * @param enabled - Whether to auto forceUpdate (default: true)
 */
export function setAutoForceUpdate(enabled: boolean): void {
  autoForceUpdate = enabled
}

/**
 * Manually trigger re-render for all Vue instances
 * Use this when you have disabled auto forceUpdate
 */
export function forceUpdateAllInstances(): void {
...
/**
 * Get the current version of dynamic components
 * This can be used to trigger reactivity
 */
export function getDynamicComponentsVersion(): number {
  return dynamicComponentsVersion
}
 ...
 */
export function registerVueInstance(vm: any): void {
  if (vm && !vueInstances.includes(vm)) {
    vueInstances.push(vm)
    debugLog(`[Vue.registerInstance] Instance registered, total: ${vueInstances.length}`)
  }
}
 ...
 * @param vm - Vue instance
 */
export function unregisterVueInstance(vm: any): void {
  const index = vueInstances.indexOf(vm)
  if (index > -1) {
 ...
 */
export function hasDynamicComponent(id: string): boolean {
 ...
 * @param id - Component name
 */
function getDynamicComponent(id: string): any {
  if (hasOwn(dynamicComponents, id)) return dynamicComponents[id]
  const camelized = camelize(id)
  if (hasOwn(dynamicComponents, camelized)) return dynamicComponents[camelized]
  const pascal = capitalize(camelized)
  if (hasOwn(dynamicComponents, pascal)) return dynamicComponents[pascal]
  return undefined
}
...
export function resolveAsset(
  ...
  if (!res && type === 'components') {
    ...
    if (dynamicComponent) {
      ...
      const shouldAutoFetch = options.autoFetchComponents !== undefined 
 ...
 */
function autoFetchComponent(componentName: string): void {
  ...
  debugLog(`[Vue.autoFetchComponent] Auto-fetching: ${componentName}`)
  ...
  if (componentName.includes('-')) {
    ...
    const folder = parts[0] // 'input'
    // Keep full component name 'input-text'
    componentPath = `/${folder}/${componentName}`
  }
  
  const basePath = config.componentPath || '/components'
  const extension = config.componentExtension || '.tpl'
  const fullPath = `${basePath}${componentPath}${extension}`
  
  debugLog(`[Vue.autoFetchComponent] Generated path: ${fullPath}`)
  ...
  fetch(fullPath)
    ...
    .then((content: string) => {
      // Parse file content
      const { script, template, loadingTemplate, style } = parseComponentFile(content)
      
      // Evaluate script
      const componentDef = evaluateScriptContent(script)
      ...
      if (style) {
        ...
        debugLog(`[Vue.autoFetchComponent] Style injected for: ${componentName}`)
      }
      ...
      debugLog(`[Vue.autoFetchComponent] ✅ Auto-registered: ${componentName}`)
    ...
    .catch((error: any) => {
      debugLog(`[Vue.autoFetchComponent] ❌ Failed to auto-fetch: ${componentName}`, error.message)
      
      // Create fallback component if not exists
      const fallbackName = config.componentFallback || 'component-notfound'
      if (!hasDynamicComponent(fallbackName)) {
        ...
        debugLog(`[Vue.autoFetchComponent] ✅ Created fallback: ${fallbackName}`)
      }
    })
    .finally(() => {
 ...
 */
const fetchingComponents: Set<string> = new Set()
 ...
 */
function evaluateScriptContent(scriptContent: string): Record<string, any> {
  try {
    ...
    warn(`[Vue.autoFetchComponent] Failed to evaluate script: ${error}`)
    return {}
  }
}
