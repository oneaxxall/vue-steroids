...
export function renderMixin(Vue: typeof Component) {
  ...
  Vue.prototype._render = function (): VNode {
    ...
    const { loadingTemplate } = vm.$options
    let finalRender = render
    let originalStaticRenderFns = vm.$options.staticRenderFns
    let swappedStaticTrees = false
    let originalStaticTrees: any = null

    if (vm.$loading && loadingTemplate) {
      if (!vm.$options._loadingRender) {
        const baseCtor = vm.$options._base as any
        if (baseCtor && baseCtor.compile) {
          try {
            const compiled = baseCtor.compile(loadingTemplate)
            vm.$options._loadingRender = compiled.render
            vm.$options._loadingStaticRenderFns = compiled.staticRenderFns
          } catch (e: any) {
            warn(`Failed to compile loadingTemplate: ${e.message}`, vm)
          }
        }
      }
