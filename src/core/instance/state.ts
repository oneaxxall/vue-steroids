 ...
 */
function initApi(vm: Component) {
  // Always initialize api namespace to prevent undefined errors
  ;(vm as any).api = {}
  
  const apiOpts = (vm.$options as any).api
  if (apiOpts && typeof apiOpts === 'object') {
    Object.keys(apiOpts).forEach(key => {
      if (typeof apiOpts[key] === 'function') {
        ;(vm as any).api[key] = apiOpts[key].bind(vm)
      }
    })
  }
}
