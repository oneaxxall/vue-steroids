...
`loadAsyncComponent` adalah method untuk **load komponen dinamis dari custom path** dengan kontrol penuh. Berbeda dengan `fetchDynamicComponent` yang menggunakan `config.componentPath`, method ini **TIDAK** menggunakan base path - Anda punya full control atas path-nya.

Namun, method ini **tetap menggunakan** `config.componentExtension` untuk auto-append extension di belakang path (bisa di-disable jika perlu).

---

## ✨ Key Features

- ✅ **Custom Path Control** - Tidak pakai base path, path sepenuhnya Anda yang atur
- ✅ **Auto Extension** - Otomatis tambah extension dari `config.componentExtension`
- ✅ **CDN/External Support** - Bisa load dari URL manapun
- ✅ **Override Extension** - Bisa pakai extension beda dari config untuk komponen tertentu
- ✅ **Callbacks** - Support `onSuccess` dan `onError` callbacks
- ✅ **Fallback Component** - Auto fallback jika file tidak ditemukan
- ✅ **No Double Extension** - Cek dulu sebelum tambah extension
...
```javascript
// Setup config (optional, untuk extension)
Vue.config.componentExtension = '.tpl'

// Load component dari custom path
...
await this.loadAsyncComponent('my-comp', '/path/component', {
  ...
  fallbackComponent: 'notfound',  // Fallback jika gagal
  onSuccess: (name, def) => {     // Callback saat berhasil
    console.log(`✅ ${name} loaded!`)
  },
  onError: (name, error) => {     // Callback saat gagal
...
|-----------|------|----------|-------------|
| `name` | `string` | ✅ Yes | Component name untuk register |
| `path` | `string` | ✅ Yes | Custom path (bisa relative, absolute, atau full URL) |
...
| `fallbackComponent` | `string` | `config.componentFallback` | Fallback component jika gagal |
| `onSuccess` | `function` | `undefined` | Callback saat berhasil load |
| `onError` | `function` | `undefined` | Callback saat gagal load |
...
new Vue({
  ...
  methods: {
    async loadComponents() {
      ...
      // Load tanpa extension di path - otomatis ditambah
      await this.loadAsyncComponent('user-card', '/components/user-card')
      
      // Path menjadi: /components/user-card.tpl
    }
  }
})
```
...
new Vue({
  ...
  methods: {
    async loadExactPath() {
      // Path sudah lengkap, tidak mau ditambah extension
      await this.loadAsyncComponent(
        'complete-component',
      ...
      // Result: /components/complete.tpl (persis seperti yang ditulis)
    }
  }
})
```
...
new Vue({
  ...
  methods: {
    async loadWithFallback() {
      // Jika file tidak ada, akan pakai fallback component
      await this.loadAsyncComponent(
        'premium-widget',
...
// fetchDynamicComponent - Pakai base path
...
```javascript
// Set default extension (digunakan oleh loadAsyncComponent)
...
await this.loadAsyncComponent('comp', '/path/comp', {
  extension: '.vue'  // Pakai .vue bukan .tpl
})
...
// ❌ BAD - Load saat dibutuhkan (bisa delay UX)
async showHeader() {
...
**Solution:**
- Path sudah ada extension, disable auto-append:
```javascript
await this.loadAsyncComponent('comp', '/path/comp.tpl', {
  autoAppendExtension: false
})
...
**Solution:**
- Make sure `autoAppendExtension` tidak di-set ke `false`:
```javascript
// Default true - akan auto-append
await this.loadAsyncComponent('comp', '/path/comp')
...
### Problem: "Load dari CDN gagal"

**Solution:**
- Check CORS policy
- Check URL是否正确
- Check network tab di DevTools
- Make sure CDN serve correct content type
...
if (success) {
  ...
  // <my-comp></my-comp> akan render
} else {
  // ❌ Failed to load
  // Fallback component akan digunakan
}
```
